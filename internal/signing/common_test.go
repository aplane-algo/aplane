...
func TestAdjustLSigFeesForDummies(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "dummy fee multiplication overflow",
			numTxns:     1,
			lsigIndices: []int{0},
			dummyCount:  2,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "fee addition overflow" {
				txns[0].Fee = types.MicroAlgos(math.MaxUint64)
			}
			...
			expectedTotal := uint64(len(tt.lsigIndices))*minFee +
				uint64(tt.dummyCount)*tt.minFee +
				tt.incentiveFee
...
func TestCalculateDummyFeesValidation(t *testing.T) {
	tests := []struct {
		name       string
		dummyCount int
		lsigCount  int
		...
		{name: "ok", dummyCount: 3, lsigCount: 2, minFee: 1000},
		{name: "negative dummy count", dummyCount: -1, lsigCount: 1, minFee: 1000, wantErr: true},
		{name: "negative lsig count", dummyCount: 1, lsigCount: -1, minFee: 1000, wantErr: true},
		{name: "overflow", dummyCount: 2, lsigCount: 1, minFee: math.MaxUint64, wantErr: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := CalculateDummyFees(tt.dummyCount, tt.lsigCount, tt.minFee)
			if (err != nil) != tt.wantErr {
				t.Fatalf("CalculateDummyFees() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
