...
func TestSimulateUnsignedTransactionsAllowsEmptySignatures(t *testing.T) {
	var gotReq models.SimulateRequest
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v2/transactions/simulate" {
			t.Fatalf("path = %s, want /v2/transactions/simulate", r.URL.Path)
		}
		body, err := io.ReadAll(r.Body)
		if err != nil {
			t.Fatalf("read request body: %v", err)
		}
		if err := msgpack.Decode(body, &gotReq); err != nil {
	...
	txns := []types.Transaction{{Header: types.Header{Sender: sender}}}
	txIDs, err := SimulateUnsignedTransactionsWithContext(
		t.Context(),
		txns,
		client,
		io.Discard,
	...
	if err != nil {
		t.Fatalf("SimulateUnsignedTransactionsWithContext() error = %v", err)
	}
	if len(txIDs) != 1 {
		t.Fatalf("len(txIDs) = %d, want 1", len(txIDs))
	}
	if !gotReq.AllowEmptySignatures {
		t.Fatal("AllowEmptySignatures = false, want true")
	}
	if !gotReq.FixSigners {
		t.Fatal("FixSigners = false, want true")
	}
	if len(gotReq.TxnGroups) != 1 || len(gotReq.TxnGroups[0].Txns) != 1 {
		t.Fatalf("simulate request groups = %#v, want one transaction", gotReq.TxnGroups)
	}
	gotTxn := gotReq.TxnGroups[0].Txns[0]
	if gotTxn.Txn.Sender != sender {
		t.Fatalf("sender = %s, want %s", gotTxn.Txn.Sender, sender)
	}
	if gotTxn.Sig != (types.Signature{}) {
		t.Fatal("unsigned simulate request included a transaction signature")
	}
}
