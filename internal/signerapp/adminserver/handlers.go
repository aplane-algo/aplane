...
func (s *Session) handleRevokeToken(msg *protocol.RevokeTokenMessage) {
	ir := s.productOrBoundRuntime()
	if !s.authorize(msg.ID, auth.ActionTokenRevoke, auth.Resource{Type: "token", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleGetAdminSettings(requestID string) {
	ir := s.productOrBoundRuntime()
	if !s.authorize(requestID, auth.ActionSettingsView, auth.Resource{Type: "settings", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleUpdateAdminSetting(msg *protocol.UpdateAdminSettingMessage) {
	ir := s.productOrBoundRuntime()
	if !s.authorize(msg.ID, auth.ActionSettingsUpdate, auth.Resource{Type: "settings", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleGetPolicySnapshot(msg *protocol.GetPolicySnapshotMessage) {
	ir := s.productOrBoundRuntime()
	if !s.authorize(msg.ID, auth.ActionPolicyView, auth.Resource{Type: "policy", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleReplacePolicy(msg *protocol.ReplacePolicyMessage) {
	ir := s.productOrBoundRuntime()
	if !s.authorize(msg.ID, auth.ActionPolicyUpdate, auth.Resource{Type: "policy", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleListSentryReferences(requestID string) {
	...
	if !s.authorize(requestID, auth.ActionSentriesView, auth.Resource{Type: "sentry_references", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleGetSentryReference(msg *protocol.GetSentryReferenceMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionSentriesView, auth.Resource{Type: "sentry_reference", ID: msg.Name, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.inspectionServices == nil {
...
func (s *Session) HandleImportSentryReference(msg *protocol.ImportSentryReferenceMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionSentriesManage, auth.Resource{Type: "sentry_reference", ID: msg.Name, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.inspectionServices == nil {
		_ = s.SendError(msg.ID, protocol.ErrCodeInternal, "store inspection service unavailable")
...
func (s *Session) HandleExportSentryPublic(msg *protocol.ExportSentryPublicMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionSentriesView, auth.Resource{Type: "sentry_public", ID: msg.WitnessKeyID, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.inspectionServices == nil {
		_ = s.SendError(msg.ID, protocol.ErrCodeInternal, "store inspection service unavailable")
...
func (s *Session) HandleListGenerations(requestID string) {
	...
	if !s.authorize(requestID, auth.ActionGenerationsView, auth.Resource{Type: "generations", IdentityID: productmode.IdentityID}) {
		return
	}
	if s.inspectionServices == nil {
		_ = s.SendError(requestID, protocol.ErrCodeInternal, "store inspection service unavailable")
...
func (s *Session) HandleUnlock(msg *protocol.UnlockMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityUnlock, auth.Resource{Type: "identity", ID: productmode.IdentityID, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleLockIdentity(msg *protocol.LockIdentityMessage) {
	...
	resource := auth.Resource{Type: "identity", ID: productmode.IdentityID, IdentityID: productmode.IdentityID}
	identity := s.Identity()
...
func (s *Session) HandleBackup(msg *protocol.BackupMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityBackup, auth.Resource{Type: "identity", ID: productmode.IdentityID, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleListBackups(requestID string) {
	...
	if !s.authorize(requestID, auth.ActionIdentityRestore, auth.Resource{Type: "identity", ID: productmode.IdentityID, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.backupServices == nil {
...
func (s *Session) HandleBeginBackupImport(msg *protocol.BeginBackupImportMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityRestore, auth.Resource{Type: "backup", ID: msg.FileName, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.backupServices == nil {
		_ = s.SendError(msg.ID, "", "backup service unavailable")
...
func (s *Session) HandleAppendBackupImport(msg *protocol.AppendBackupImportMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityRestore, auth.Resource{Type: "backup_upload", ID: msg.UploadID, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.backupServices == nil {
		_ = s.SendError(msg.ID, "", "backup service unavailable")
...
func (s *Session) HandleReadBackupChunk(msg *protocol.ReadBackupChunkMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityBackup, auth.Resource{Type: "backup", ID: msg.FileName, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.backupServices == nil {
		_ = s.SendError(msg.ID, "", "backup service unavailable")
	...
	if result.Success {
		if audit, ok := s.audit.(interface {
			...
			if s.markBackupExportChunk(productmode.IdentityID, result.FileName, result.Offset, result.EOF) {
...
func (s *Session) HandleChangeStorePassphrase(msg *protocol.ChangeStorePassphraseMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityPassphrase, auth.Resource{Type: "identity", ID: productmode.IdentityID, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandlePreviewRestore(msg *protocol.PreviewRestoreMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionIdentityRestore, auth.Resource{Type: "identity", ID: productmode.IdentityID, IdentityID: productmode.IdentityID}) {
		return
	}
	if s.backupServices == nil {
		_ = s.SendError(msg.ID, "", "backup service unavailable")
...
func (s *Session) HandleListKeys(requestID string) {
	...
	if !s.authorize(requestID, auth.ActionKeysView, auth.Resource{Type: "keys", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleGetKeyDetails(msg *protocol.GetKeyDetailsMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionKeysView, auth.Resource{Type: "key", ID: msg.Address, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleListLibraryTemplates(requestID string) {
	...
	if !s.authorize(requestID, auth.ActionTemplatesView, auth.Resource{Type: "templates", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleInstallLibraryTemplate(msg *protocol.InstallLibraryTemplateMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionTemplatesInstall, auth.Resource{Type: "template", ID: msg.KeyType, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleShowInstalledTemplate(msg *protocol.ShowInstalledTemplateMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionTemplatesView, auth.Resource{Type: "template", ID: msg.KeyType, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleImportInstalledTemplate(msg *protocol.ImportInstalledTemplateMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionTemplatesInstall, auth.Resource{Type: "template", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleRemoveInstalledTemplate(msg *protocol.RemoveInstalledTemplateMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionTemplatesRemove, auth.Resource{Type: "template", ID: msg.KeyType, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleActivateKeyType(msg *protocol.ActivateKeyTypeMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionKeyTypesActivate, auth.Resource{Type: "keytype", ID: msg.KeyType, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleDeactivateKeyType(msg *protocol.DeactivateKeyTypeMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionKeyTypesDeactivate, auth.Resource{Type: "keytype", ID: msg.KeyType, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleListKeyTypes(requestID string) {
	...
	if !s.authorize(requestID, auth.ActionKeyTypesView, auth.Resource{Type: "keytypes", IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleSignResponse(msg *protocol.SignResponseMessage) {
	if ir := s.BoundRuntime(); ir != nil {
		if !s.authorize(msg.ID, auth.ActionSignApprove, auth.Resource{Type: "sign_request", ID: msg.ID, IdentityID: productmode.IdentityID}) {
			return
		}
...
func (s *Session) HandleTokenProvisioningResponse(msg *protocol.TokenProvisioningResponseMessage) {
	if ir := s.BoundRuntime(); ir != nil {
		if !s.authorize(msg.ID, auth.ActionTokenProvision, auth.Resource{Type: "token_provisioning", ID: msg.ID, IdentityID: productmode.IdentityID}) {
			return
		}
...
func (s *Session) HandleGenerateKey(msg *protocol.GenerateKeyMessage) {
	...
	resource := auth.Resource{Type: "key", IdentityID: productmode.IdentityID}
...
func (s *Session) HandleDeleteKey(msg *protocol.DeleteKeyMessage) {
	...
	if !s.authorize(msg.ID, auth.ActionKeysDelete, auth.Resource{Type: "key", ID: msg.Address, IdentityID: productmode.IdentityID}) {
		return
	}
...
func (s *Session) HandleExportKey(msg *protocol.ExportKeyMessage) {
	...
	resource := auth.Resource{Type: "key", ID: msg.Address, IdentityID: productmode.IdentityID}
