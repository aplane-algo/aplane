...
func logHTTPStartup(keyCount int, keysSnapshot map[string]string, port int) {
	...
	logInfof("  POST   /sign/bounded-component   - Produce approved bounded base components")
	logInfof("  POST   /sign/bounded-assemble    - Assemble bounded-sentry signed groups")
	...
	logInfof("  POST   /admin/sentries/sync    - Sync public sentry references")
