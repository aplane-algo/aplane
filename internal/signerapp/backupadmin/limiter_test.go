...
func TestRestoreAttemptLimiterBackoffAndReset(t *testing.T) {
	...
	if got := limiter.RetryAfter("/backup.tar.gz"); got != 0 {
		t.Fatalf("initial retry after = %v, want 0", got)
	}

	limiter.RecordFailure("/backup.tar.gz")
	if got := limiter.RetryAfter("/backup.tar.gz"); got != time.Second {
	...
	limiter.RecordFailure("/backup.tar.gz")
	if got := limiter.RetryAfter("/backup.tar.gz"); got != 2*time.Second {
		t.Fatalf("second retry after = %v, want 2s", got)
	}

	limiter.RecordSuccess("/backup.tar.gz")
	if got := limiter.RetryAfter("/backup.tar.gz"); got != 0 {
