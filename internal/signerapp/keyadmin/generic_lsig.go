...
func (g GenericLSigGenerator) GenerateContext(ctx context.Context, ir *identity.Runtime, keyType string, parameters map[string]string) (string, error) {
	...
	if err := ir.WithMasterKey(func(mk []byte) error {
		return mut.SaveGenericLSig(address, keyType, template.RoutingFamily(), parameters, bytecode, saltCounter, tealSource, signingArgs, mk)
	}); err != nil {
	...
	if g.AuditLog != nil {
		g.AuditLog.LogKeyGenerated(ir.ID(), address, keyType)
	}
