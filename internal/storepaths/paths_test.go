...
func TestKeyTypePathsAreIdentityScoped(t *testing.T) {
	...
	if gotFile := paths.KeyTypeTemplate("default", "aplane.whitelist.v1"); gotFile != filepath.Join(wantDir, "aplane.whitelist.v1.template") {
...
func TestDeletedPathsAreIdentityScoped(t *testing.T) {
	...
	got := paths.DeletedKeyTypeTemplate("default", "aplane.whitelist.v1")
	want := filepath.Join(identityDir, "deleted", "keytypes", "aplane.whitelist.v1.template")
	if got != want {
