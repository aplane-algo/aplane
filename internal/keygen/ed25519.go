...
func (g *Ed25519Generator) GenerateFromSeed(ctx context.Context, paths storepaths.Paths, seed []byte, kr *securecrypto.Keyring, keyType string, params map[string]string) (*GenerationResult, error) {
	_ = ctx
	_ = params
	if keyType != "ed25519" {
	...
	// Save key files
	keyFiles, err := saveEd25519Keys(paths, publicKey, privateKey, kr)
	if err != nil {
...
// keyType must be "ed25519".
func (g *Ed25519Generator) GenerateFromMnemonic(ctx context.Context, paths storepaths.Paths, mnemonicStr string, kr *securecrypto.Keyring, keyType string, params map[string]string) (*GenerationResult, error) {
	_ = params
	if keyType != "ed25519" {
		return nil, fmt.Errorf("ed25519 generator only supports keyType \"ed25519\", got %q", keyType)
	}
	...
	result, err := g.GenerateFromSeed(ctx, paths, seed, kr, keyType, nil)
	if err != nil {
		return nil, err
	}
...
// keyType must be "ed25519".
func (g *Ed25519Generator) GenerateRandom(ctx context.Context, paths storepaths.Paths, kr *securecrypto.Keyring, keyType string, params map[string]string) (*GenerationResult, error) {
	_ = ctx
	_ = params
	if keyType != "ed25519" {
		return nil, fmt.Errorf("ed25519 generator only supports keyType \"ed25519\", got %q", keyType)
	...
	// Save key files
	keyFiles, err := saveEd25519Keys(paths, account.PublicKey, account.PrivateKey, kr)
	if err != nil {
		return nil, fmt.Errorf("failed to save keys: %w", err)
	}
...
func saveEd25519Keys(paths storepaths.Paths, publicKey []byte, privateKey []byte, kr *securecrypto.Keyring) (*keys.ImportKeyResult, error) {
	payload := keys.NewEd25519Payload(publicKey, privateKey)
	defer payload.ZeroSecrets()
	return keys.SavePayload(paths, payload, kr)
}
