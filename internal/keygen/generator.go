...
type Generator interface {
	...
	GenerateFromSeed(ctx context.Context, paths storepaths.Paths, seed []byte, kr *crypto.Keyring, keyType string, params map[string]string) (*GenerationResult, error)

	// GenerateFromMnemonic generates a key from mnemonic words.
	// keyType must be explicitly specified (e.g., "aplane.falcon1024.v1", "ed25519").
	GenerateFromMnemonic(ctx context.Context, paths storepaths.Paths, mnemonic string, kr *crypto.Keyring, keyType string, params map[string]string) (*GenerationResult, error)

	// GenerateRandom generates a new random key.
	// keyType must be explicitly specified (e.g., "aplane.falcon1024.v1", "ed25519").
	GenerateRandom(ctx context.Context, paths storepaths.Paths, kr *crypto.Keyring, keyType string, params map[string]string) (*GenerationResult, error)
}
