...
func TestFalcon1024V1Registration(t *testing.T) {
	...
	if dsa.KeyType() != "aplane.falcon1024.v1" {
...
func TestGetKeyTypes(t *testing.T) {
	...
	if !found {
		t.Errorf("aplane.falcon1024.v1 not in GetKeyTypes(): %v", keyTypes)
	}
}
...
func TestGetAll(t *testing.T) {
	...
	if !found {
		t.Error("aplane.falcon1024.v1 not in GetAll()")
	}
}
...
func TestFalcon1024V1GenerateAndDerive(t *testing.T) {
	...
	// Generate keypair
	pub, priv, err := signerops.New(nil).GenerateKeypair(seed)
	if err != nil {
	...
	// Derive LogicSig
	bytecode, addr, err := dsa.DeriveLsig(context.Background(), pub, nil)
	if err != nil {
...
func TestFalcon1024V1Sign(t *testing.T) {
	...
	// Generate keypair
	_, priv, err := signerops.New(nil).GenerateKeypair(seed)
	if err != nil {
		t.Fatalf("GenerateKeypair failed: %v", err)
	}
	...
	sig, err := signerops.New(nil).Sign(priv, message)
	if err != nil {
...
func TestIsLogicSigType(t *testing.T) {
	tests := []struct {
		...
		{"aplane.falcon1024.v1", true}, // Directly registered
...
func TestConfigureAlgodClient(t *testing.T) {
	...
	// Verify aplane.falcon1024.v1 (which implements AlgodConfigurable) is configured
	dsa := logicsigdsa.Get("aplane.falcon1024.v1")
	if dsa == nil {
		t.Fatal("aplane.falcon1024.v1 not registered")
	}
	...
	if _, ok := dsa.(lsigprovider.AlgodConfigurable); !ok {
		t.Error("aplane.falcon1024.v1 should implement AlgodConfigurable")
	}
	...
	pub, _, err := signerops.New(nil).GenerateKeypair(seed)
	if err != nil {
