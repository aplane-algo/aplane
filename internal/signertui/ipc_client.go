...
func (c *IPCClient) forwardMessages(sessionID uint64, done <-chan struct{}, notifications <-chan transport.Notification, lifecycle <-chan transport.LifecycleEvent) {
	...
	for {
		...
		case notification, ok := <-notifications:
			...
			case MsgTypePolicySettings:
				var settings PolicySettingsMessage
				if err := json.Unmarshal(line, &settings); err != nil {
					continue
				}
				c.emit(sessionID, PolicySettingsMsg{
					Settings: PolicySettings{
						...
						ReviewAlgoPayments:          cloneStringMap(settings.ReviewAlgoPayments),
						MaxAlgoPayments:             cloneStringMap(settings.MaxAlgoPayments),
						PolicyNetworks:              append([]string(nil), settings.PolicyNetworks...),
						ReviewASAAmounts:            cloneStringMap(settings.ReviewASAAmounts),
						MaxASAAmounts:               policyASAAmountsFromMessage(settings),
						PolicyASAMetadata:           clonePolicyASAMetadata(settings.PolicyASAMetadata),
			...
			case MsgTypeReplacePolicyResult:
				var result ReplacePolicyResultMessage
				if err := json.Unmarshal(line, &result); err != nil {
					continue
				}
			...
			case MsgTypeASAMetadataResults:
				var result ASAMetadataResultsMessage
				if err := json.Unmarshal(line, &result); err != nil {
					continue
				}
				c.emit(sessionID, ASAMetadataResultsMsg{
			...
			case MsgTypeASAMetadataResult:
				var result ASAMetadataResultMessage
				if err := json.Unmarshal(line, &result); err != nil {
					continue
				}
				var asset ASAMetadataInfo
				errText := result.Error
				if result.Asset == nil && errText == "" {
					errText = "ASA metadata response missing asset"
				}
				if result.Asset != nil {
					asset = *result.Asset
				}
				c.emit(sessionID, ASAMetadataResultMsg{
					Network: result.Network,
					Asset:   asset,
					Error:   errText,
				})

			case MsgTypeUpdatePolicySettingResult:
				var result UpdatePolicySettingResultMessage
				if err := json.Unmarshal(line, &result); err != nil {
					continue
				}
				c.emit(sessionID, PolicySettingUpdatedMsg{
					Success: result.Success,
					Key:     result.Key,
					Value:   result.Value,
					Error:   result.Error,
			...
			case MsgTypeUpdatePolicyASAResult:
				var result UpdatePolicyASAResultMessage
				if err := json.Unmarshal(line, &result); err != nil {
					continue
				}
				c.emit(sessionID, PolicySettingUpdatedMsg{
					Success: result.Success,
					Key:     policyPanelActionTransferGuards,
					Error:   result.Error,
				})
...
// SendGetPolicySettings sends a request to get the current policy settings.
func (c *IPCClient) SendGetPolicySettings() error {
	msg := GetPolicySettingsMessage{
		BaseMessage: BaseMessage{
			Type: MsgTypeGetPolicySettings,
...
// SendGetPolicySnapshot sends a request to get the active read-only policy snapshot.
func (c *IPCClient) SendGetPolicySnapshot() error {
...
func (c *IPCClient) SendReplacePolicy(policyYAML, expectedCurrentSHA256 string) error {
...
// SendUpdatePolicySetting sends a request to update a single policy setting.
func (c *IPCClient) SendUpdatePolicySetting(key, value string) error {
	msg := UpdatePolicySettingMessage{
		BaseMessage: BaseMessage{
			Type: MsgTypeUpdatePolicySetting,
			ID:   fmt.Sprintf("policy-set-%d", time.Now().UnixNano()),
		},
		Key:   key,
		Value: value,
	}
...
// SendUpdatePolicyASAAmounts sends an atomic request to update network transfer guards.
func (c *IPCClient) SendUpdatePolicyASAAmounts(reviewAmounts map[string]string, maxAmounts map[string]string, reviewAlgoPayments map[string]string, maxAlgoPayments map[string]string) error {
	msg := UpdatePolicyASAAmountsMessage{
		BaseMessage: BaseMessage{
			Type: MsgTypeUpdatePolicyASAAmounts,
			ID:   fmt.Sprintf("policy-asa-%d", time.Now().UnixNano()),
		},
		ReviewASAAmounts:   cloneStringMap(reviewAmounts),
		MaxASAAmounts:      cloneStringMap(maxAmounts),
		ReviewAlgoPayments: cloneStringMap(reviewAlgoPayments),
		MaxAlgoPayments:    cloneStringMap(maxAlgoPayments),
		Mainnet:            maxAmounts["mainnet"],
		Testnet:            maxAmounts["testnet"],
		Betanet:            maxAmounts["betanet"],
	}
	return c.sendMessage(msg)
}

func (c *IPCClient) SendSearchASAMetadata(network, query string) error {
	msg := SearchASAMetadataMessage{
		BaseMessage: BaseMessage{
			Type: MsgTypeSearchASAMetadata,
			ID:   fmt.Sprintf("asa-search-%d", time.Now().UnixNano()),
		},
		Network: network,
		Query:   query,
	}
...
func (c *IPCClient) SendResolveASAMetadata(network string, assetID uint64) error {
	msg := ResolveASAMetadataMessage{
		BaseMessage: BaseMessage{
			Type: MsgTypeResolveASAMetadata,
			ID:   fmt.Sprintf("asa-resolve-%d", time.Now().UnixNano()),
		},
		Network: network,
		AssetID: assetID,
	}
...
// SendGetPolicySettingsCmd returns a tea.Cmd that requests policy settings.
func (m Model) sendGetPolicySettingsCmd() tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error { return c.SendGetPolicySettings() })
}

// SendGetPolicySnapshotCmd returns a tea.Cmd that requests the active read-only policy snapshot.
func (m Model) sendGetPolicySnapshotCmd() tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error { return c.SendGetPolicySnapshot() })
}

func (m Model) sendReplacePolicyCmd(policyYAML, expectedCurrentSHA256 string) tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error {
		return c.SendReplacePolicy(policyYAML, expectedCurrentSHA256)
	})
}

// SendUpdatePolicySettingCmd returns a tea.Cmd that updates a policy setting.
func (m Model) sendUpdatePolicySettingCmd(key, value string) tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error { return c.SendUpdatePolicySetting(key, value) })
}

// SendUpdatePolicyASAAmountsCmd returns a tea.Cmd that atomically updates network transfer guards.
func (m Model) sendUpdatePolicyASAAmountsCmd(reviewAmounts map[string]string, maxAmounts map[string]string, reviewAlgoPayments map[string]string, maxAlgoPayments map[string]string) tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error {
		return c.SendUpdatePolicyASAAmounts(reviewAmounts, maxAmounts, reviewAlgoPayments, maxAlgoPayments)
	})
}

func (m Model) sendSearchASAMetadataCmd(network, query string) tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error { return c.SendSearchASAMetadata(network, query) })
}

func (m Model) sendResolveASAMetadataCmd(network string, assetID uint64) tea.Cmd {
	return ipcCmd(m.adminClient, func(c *IPCClient) error { return c.SendResolveASAMetadata(network, assetID) })
}

func policyASAAmountsFromMessage(settings PolicySettingsMessage) map[string]string {
	amounts := cloneStringMap(settings.MaxASAAmounts)
	if amounts == nil {
		amounts = make(map[string]string, 3)
	}
	if settings.MaxASAAmountsMainnet != "" {
		amounts["mainnet"] = settings.MaxASAAmountsMainnet
	}
	if settings.MaxASAAmountsTestnet != "" {
		amounts["testnet"] = settings.MaxASAAmountsTestnet
	}
	if settings.MaxASAAmountsBetanet != "" {
		amounts["betanet"] = settings.MaxASAAmountsBetanet
	}
	return amounts
}

func clonePolicyASAMetadata(in map[string][]ASAMetadataInfo) map[string][]ASAMetadataInfo {
	if in == nil {
		return nil
	}
	out := make(map[string][]ASAMetadataInfo, len(in))
	for network, items := range in {
		out[network] = append([]ASAMetadataInfo(nil), items...)
	}
	return out
}
