...
import (
	...
	util "github.com/aplane-algo/aplane/internal/checksum"
)
...
func TestVerifyPluginManifestNotInChecksums(t *testing.T) {
	tmpDir := t.TempDir()
	...
	content, err := GenerateChecksums(tmpDir, []string{"plugin"})
	if err != nil {
		t.Fatal(err)
	}
	checksumPath := filepath.Join(tmpDir, "checksums.sha256")
	...
	verifier := NewVerifier()
	err = verifier.VerifyPlugin(tmpDir, "./plugin")
	if !errors.Is(err, ErrManifestNotInChecksums) {
		t.Errorf("Expected ErrManifestNotInChecksums, got %v", err)
	}
}
...
func TestVerifyPluginRejectsPathTraversalInChecksums(t *testing.T) {
	tmpDir := t.TempDir()
	...
	content += strings.Repeat("a", 64) + "  ../../etc/passwd\n"
	if err := os.WriteFile(checksumPath, []byte(content), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if !errors.Is(err, ErrChecksumsPathEscape) {
		t.Fatalf("expected ErrChecksumsPathEscape, got %v", err)
	}
}
