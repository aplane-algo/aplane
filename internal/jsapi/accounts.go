...
func (a *API) jsAddAlias(call goja.FunctionCall) goja.Value {
	...
	result, err := a.engine.AddAliasWithContext(a.Context(), name, address)
	if err != nil {
...
func (a *API) jsSigners(call goja.FunctionCall) goja.Value {
	allSigners, err := a.engine.ListSigners()
	if err != nil {
		panic(a.runtime.ToValue(fmt.Sprintf("signers() error: %v", err)))
	}
