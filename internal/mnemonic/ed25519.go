...
func (h *Ed25519Handler) GenerateMnemonic() (words string, seed []byte, entropy []byte, err error) {
	...
	// For Ed25519, return a dedicated copy of the private key so the account-owned
	// buffer can be zeroed before returning. Callers remain responsible for wiping
	// the returned seed when finished with it.
	seed = append([]byte(nil), account.PrivateKey...)
	return mnemonicStr, seed, nil, nil
}
...
func (h *Ed25519Handler) SeedFromMnemonic(words []string, passphrase string) ([]byte, error) {
	...
	// Return a dedicated copy so the SDK-owned buffer can be wiped before return.
	// Callers remain responsible for zeroing the returned seed.
