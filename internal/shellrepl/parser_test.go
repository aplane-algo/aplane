...
func TestParseSendCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "send with text prefix arg",
			args:    []string{"1", "algo", "from", "alice", "to", "bob", "arg:preimage=text:hello"},
			wantErr: false,
			checkFunc: func(t *testing.T, p TransactionParams) {
				if !bytes.Equal(p.LsigArgs["preimage"], []byte("hello")) {
					t.Errorf("LsigArgs[preimage] = %v, want %v", p.LsigArgs["preimage"], []byte("hello"))
		...
		{
			name:    "send with base64 arg",
			args:    []string{"1", "algo", "from", "alice", "to", "bob", "arg:preimage=b64:aGVsbG8="},
			wantErr: false,
			checkFunc: func(t *testing.T, p TransactionParams) {
				if !bytes.Equal(p.LsigArgs["preimage"], []byte("hello")) {
					t.Errorf("LsigArgs[preimage] = %v, want %v", p.LsigArgs["preimage"], []byte("hello"))
		...
		{
			name:    "send with duplicate fee",
			args:    []string{"1", "algo", "from", "alice", "to", "bob", "fee=1000", "fee=2000"},
			wantErr: true,
		},
		{
		...
		{
			name:    "send with bracketed senders and receivers",
			args:    []string{"1", "algo", "from", "[alice", "bob]", "to", "[carol", "dave]"},
			wantErr: false,
			checkFunc: func(t *testing.T, p TransactionParams) {
				if len(p.FromRaw) != 2 || p.FromRaw[0] != "alice" || p.FromRaw[1] != "bob" {
					t.Errorf("FromRaw = %v, want [alice bob]", p.FromRaw)
				}
				if len(p.ToRaw) != 2 || p.ToRaw[0] != "carol" || p.ToRaw[1] != "dave" {
					t.Errorf("ToRaw = %v, want [carol dave]", p.ToRaw)
				}
			},
		},
		{
		...
		{
			name:    "send allows receiver alias named atomic",
			args:    []string{"1", "algo", "from", "alice", "to", "atomic"},
			wantErr: false,
			checkFunc: func(t *testing.T, p TransactionParams) {
				if p.Atomic {
					t.Error("Atomic should remain false when receiver is named atomic")
				}
				if len(p.ToRaw) != 1 || p.ToRaw[0] != "atomic" {
					t.Errorf("ToRaw = %v, want [atomic]", p.ToRaw)
				}
			},
		},
		{
			name:    "send note value does not become flag",
			args:    []string{"1", "algo", "from", "alice", "to", "bob", "note", "atomic"},
			wantErr: false,
			checkFunc: func(t *testing.T, p TransactionParams) {
				if p.Note != "atomic" {
					t.Errorf("Note = %q, want atomic", p.Note)
				}
				if p.Atomic {
					t.Error("Atomic should remain false when note value is atomic")
				}
			},
		},
		{
			name:    "send allows bracketed sender named to",
			args:    []string{"1", "algo", "from", "[", "to", "]", "to", "bob"},
			wantErr: false,
			checkFunc: func(t *testing.T, p TransactionParams) {
				if len(p.FromRaw) != 1 || p.FromRaw[0] != "to" {
					t.Errorf("FromRaw = %v, want [to]", p.FromRaw)
				}
			},
		},
	}
...
func TestParseOptoutCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "optout allows asset named from",
			args:    []string{"from", "from", "alice"},
			wantErr: false,
			checkFunc: func(t *testing.T, p OptOutParams) {
				if p.ASARef != "from" {
					t.Errorf("ASARef = %v, want from", p.ASARef)
				}
				if p.Account != "alice" {
					t.Errorf("Account = %v, want alice", p.Account)
				}
		...
		{
			name:    "optout allows account named to",
			args:    []string{"usdc", "from", "to"},
			wantErr: false,
			checkFunc: func(t *testing.T, p OptOutParams) {
				if p.Account != "to" {
					t.Errorf("Account = %v, want to", p.Account)
				}
				if p.CloseTo != "" {
					t.Errorf("CloseTo = %v, want empty", p.CloseTo)
				}
		...
		{
			name:    "optout ignores flag-like to target",
			args:    []string{"usdc", "from", "alice", "to", "fee=1000"},
			wantErr: false,
			checkFunc: func(t *testing.T, p OptOutParams) {
				if p.CloseTo != "" {
					t.Errorf("CloseTo = %v, want empty", p.CloseTo)
...
func TestParseCloseCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "close allows account named to",
			args:    []string{"to", "to", "bob"},
			wantErr: false,
			checkFunc: func(t *testing.T, p CloseParams) {
				if p.Account != "to" {
					t.Errorf("Account = %v, want to", p.Account)
...
func TestParseRekeyCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "rekey allows account named to",
			args:      []string{"to", "to", "bob"},
			isUnrekey: false,
			wantErr:   false,
			checkFunc: func(t *testing.T, p RekeyParams) {
		...
		},
		{
			...
			args:      []string{"alice", "to", "bob", "using", "contract-admin", "/keys/cold.wit"},
			isUnrekey: false,
			wantErr:   true,
		},
		{
...
func TestParseSweepCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "sweep allows destination named leaving",
			args:    []string{"algo", "to", "leaving"},
			wantErr: false,
			checkFunc: func(t *testing.T, p SweepParams) {
				if p.ToRaw != "leaving" {
					t.Errorf("ToRaw = %q, want leaving", p.ToRaw)
				}
				if p.Leaving != "0" {
					t.Errorf("Leaving = %v, want 0", p.Leaving)
				}
		...
		{
			name:    "sweep allows bracketed source named to",
			args:    []string{"algo", "from", "[", "to", "]", "to", "treasury"},
			wantErr: false,
			checkFunc: func(t *testing.T, p SweepParams) {
				if len(p.FromRaw) != 1 || p.FromRaw[0] != "to" {
					t.Errorf("FromRaw = %v, want [to]", p.FromRaw)
...
func TestParseAccountSet(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			accounts, endIdx, err := cmdspec.ExtractBracketList(tt.args, tt.startIdx)
			if (err != nil) != tt.wantErr {
				t.Errorf("ExtractBracketList() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !tt.wantErr {
				if len(accounts) != tt.wantCount {
					t.Errorf("ExtractBracketList() count = %v, want %v", len(accounts), tt.wantCount)
				}
				if endIdx != tt.wantEndIdx {
					t.Errorf("ExtractBracketList() endIdx = %v, want %v", endIdx, tt.wantEndIdx)
				}
			}
		})
	}
