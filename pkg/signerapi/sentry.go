...
// shape the mixed guarded flow sends to /sign: positions this signer signs
...
// Validate checks the request shape that can be validated without signer state.
// Every group position must be covered exactly once: by a guarded target, a
// signed passthrough entry, or a local sign-mode request (auth_address set).
