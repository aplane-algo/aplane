...
- `max_opcode_cost` is required. It is the reviewed worst-case cost of the
  final compiled and auto-salted program across every permitted authorization
  path and runtime input. Installation rejects a missing or zero declaration;
  APlane never invents an opcode ceiling for template authors.
