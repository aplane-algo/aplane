...
from tools import load_swap_app_id, TOOL_SCHEMAS, dispatch_tool
...
def run(role: str, my_address: str, peer_address: str, state_path: str,
    ...
    if state is None:
        state = SwapState(
            ...
            swap_app_id=load_swap_app_id(),
            last_seen_round=_get_current_round(),
        )
        save_state(state, state_path)

    print(f"=== Atomic Group Swap Orchestrator ({role}) ===")
    ...
    print(f"  I offer:      {my_label} amount {state.my_asa_amount}")
    print(f"  Peer offers:  {peer_label} amount {state.peer_asa_amount}")
    print()
    tag = my_address[:4]
    log(tag, f"Started {role} — offering {my_label} (amount {state.my_asa_amount})")
    ...
    fmt = dict(
        ...
        my_asa_label=my_label,
        peer_asa_label=peer_label,
    )
    if role == "buyer":
        fmt["swap_app_id"] = state.swap_app_id or load_swap_app_id()
