...
const RETI_APP_IDS: Record<string, bigint> = {
    testnet: 734834614n,
    mainnet: 2714516089n,
    betanet: 639070n
};
...
async function getValidatorClient(sender: string = FEE_SINK): Promise<ValidatorRegistryClient> {
    if (!pluginState.algorand) throw new Error('Not initialized');
...
async function getStakingPoolClient(poolAppId: bigint, sender: string = FEE_SINK): Promise<StakingPoolClient> {
    if (!pluginState.algorand) throw new Error('Not initialized');
...
async function handleExecute(params: any): Promise<ExecuteResult> {
    if (!pluginState.initialized) throw new Error('Plugin not initialized');
...
async function handleList(args: string[]): Promise<ExecuteResult> {
    ...
    const validators: Array<{id: number, name: string, commission: number, totalStaked: number, numPools: number, totalStakers: number, minEntryStake: number}> = [];
...
async function handlePools(args: string[]): Promise<ExecuteResult> {
    if (args.length !== 1) throw new Error('Usage: reti pools <validator_id>');
    ...
    const pools: Array<{id: number, appId: number, stakersCount: number, balance: number}> = [];
...
async function hasExistingStake(userAddr: string, validatorId: bigint): Promise<boolean> {
    try {
...
async function handleDeposit(args: string[], context: Context): Promise<ExecuteResult> {
    ...
    if (needsMbr) totalPayment += mbrAmounts.addStakerMbr;
    ...
    const appBudgetAdded = simulateResults.simulateResponse.txnGroups[0].appBudgetAdded as number || 0;
    const feeAmount = AlgoAmount.MicroAlgos(
        1000 * Math.floor((appBudgetAdded + 699) / 700) - 1000
    );
    ...
    // Encode for apshell
    const transactions: TransactionIntent[] = txns.map((txn, i) => ({
        type: 'raw',
...
async function handleWithdraw(args: string[], context: Context): Promise<ExecuteResult> {
    ...
    const appBudgetAdded = simulateResult.simulateResponse.txnGroups[0].appBudgetAdded as number || 0;
    const feeAmount = AlgoAmount.MicroAlgos(
        1000 * Math.floor((appBudgetAdded + 699) / 700) - 2000
    );
...
async function handleBalance(args: string[], context: Context): Promise<ExecuteResult> {
    ...
    let stakerPools: [bigint, bigint, bigint][];
    ...
    let totalStaked = 0n;
    const stakes: Array<{poolAppId: number, validatorId: number, balance: number}> = [];
...
rl.on('line', async (line: string) => {
    let request: JsonRpcRequest = { jsonrpc: '2.0', id: 0, method: '', params: {} };
    try {
        ...
        switch (method) {
            ...
            case 'execute': result = await handleExecute(params); break;
            case 'getInfo': result = handleGetInfo(); break;
            case 'shutdown':
