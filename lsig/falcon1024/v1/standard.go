...
func (f *Falcon1024V1) DeriveLsig(ctx context.Context, publicKey []byte, params map[string]string) (lsigBytecode []byte, address string, err error) {
...
func (f *Falcon1024V1) DeriveLsigWithSalt(ctx context.Context, publicKey []byte, params map[string]string) (lsigsalt.FindResult, error) {
	_ = params // Pure Falcon ignores params
	if len(publicKey) != family.PublicKeySize {
		return lsigsalt.FindResult{}, fmt.Errorf("invalid public key size: expected %d, got %d",
			family.PublicKeySize, len(publicKey))
	}
...
func (f *Falcon1024V1) Description() string {
	return "Falcon-1024 signature scheme"
}
...
func (f *Falcon1024V1) SupportsMnemonicImport() bool {
	return true
}
...
var (
	...
	_ logicsigdsa.SaltedDeriver     = (*Falcon1024V1)(nil)
