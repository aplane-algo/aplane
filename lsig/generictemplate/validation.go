...
// correct names, types, and constraints. Used by both generictemplate and
// composeddsa validation.
func ValidateParameterSpecs(params []ParameterSpec) error {
	...
	for _, p := range params {
		...
		switch p.Type {
		case "address", "address[]", "uint64", "uint64[]", "bytes":
			// Valid types
		default:
		...
		if p.Type == "address[]" || p.Type == "uint64[]" {
			if p.MinItems < 0 || p.MaxItems < 0 {
				return fmt.Errorf("parameter %s: min_items/max_items cannot be negative", p.Name)
			}
			if p.MaxItems > 0 && p.MinItems > p.MaxItems {
				return fmt.Errorf("parameter %s: min_items (%d) cannot be greater than max_items (%d)", p.Name, p.MinItems, p.MaxItems)
			}
			if p.Min != nil || p.Max != nil {
				return fmt.Errorf("parameter %s: min/max constraints are not valid for %s", p.Name, p.Type)
			}
		}
...
// Used by both generictemplate and composeddsa validation.
...
func ValidateParameterValues(params map[string]string, defs []lsigprovider.ParameterDef) error {
	...
	for _, def := range defs {
		...
		if def.IsList() {
			if err := validateListParameterValue(value, def); err != nil {
...
func validateListParameterValue(value string, def lsigprovider.ParameterDef) error {
	if !def.IsList() {
		return fmt.Errorf("parameter %s is not list-typed", def.Name)
	}
	...
	for _, part := range parts {
		...
		if _, ok := seen[item]; ok {
			return fmt.Errorf("duplicate list item: %s", item)
		}
		seen[item] = struct{}{}
		items = append(items, item)
	}

	if def.MinItems > 0 && len(items) < def.MinItems {
		return fmt.Errorf("expected at least %d items, got %d", def.MinItems, len(items))
	}
	if def.MaxItems > 0 && len(items) > def.MaxItems {
		return fmt.Errorf("expected at most %d items, got %d", def.MaxItems, len(items))
	}

	elemType := def.ElementType()
	for _, item := range items {
		if err := ValidateParameterValue(item, elemType, 0); err != nil {
			return err
		}
	}
	return nil
