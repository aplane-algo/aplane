...
func TestBundledBoundedCompiledBudgetMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "aplane.falcon1024-allowlist.v1.yaml", publicKey: bytes.Repeat([]byte{0x21}, falconfamily.PublicKeySize), parameters: map[string]string{"recipients": strings.Join(recipients, ",")}, bytecode: 3159, spend: 4439, spendGroup: 5, group: 5, spendMinFeeCeiling: 2000},
		{name: "aplane.falcon1024-allowlist.v2.yaml", publicKey: bytes.Repeat([]byte{0x21}, falconfamily.PublicKeySize), parameters: map[string]string{"recipients": recipients[0]}, bytecode: 2188, spend: 3980, spendGroup: 4, group: 4, spendMinFeeCeiling: 2500},
		{name: "aplane.falcon1024-timelock.v1.yaml", publicKey: bytes.Repeat([]byte{0x21}, falconfamily.PublicKeySize), parameters: map[string]string{"unlock_round": "18446744073709551615"}, bytecode: 1947, spend: 3227, spendGroup: 4, group: 4, spendMinFeeCeiling: 2500},
		...
		}, bytecode: 5312, spend: 6592, admin: 7872, spendGroup: 7, adminGroup: 8, group: 8, spendMinFeeCeiling: 1428, adminMinFeeCeiling: 1250},
		{name: "aplane.corridor.v1.yaml", publicKey: bytes.Repeat([]byte{0x21}, falconfamily.PublicKeySize), parameters: map[string]string{
			...
			composeddsa.BoundedAdminPublicKeyParameter:  hex.EncodeToString(adminKey),
		}, bytecode: 5940, spend: 9012, admin: 8500, spendGroup: 10, adminGroup: 9, group: 10, spendMinFeeCeiling: 1000, adminMinFeeCeiling: 1111},
	}
