...
13. **apkey-migrate** - standalone key-file state repair utility
...
CGO_ENABLED=0 go build -o bin/apkey-migrate ./cmd/apkey-migrate
...
`connect` reads the SSH target from `$APCLIENT_DATA/config.yaml`; it does not
