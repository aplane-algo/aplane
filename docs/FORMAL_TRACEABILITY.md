...
| R4 | implemented | Rotation never erases pre-cutover divergence | `internal/rotationinventory/baseline.go::EvaluateRollbackCutover`; `::EvaluateRollback`; `internal/rotationinventory/start.go::StartRotation`; `internal/rotationinventory/complete.go::CompleteRotation` | `internal/rotationinventory/complete_test.go::TestCompleteRotationDoesNotEraseCutoverDivergence`; `internal/rotationinventory/baseline_test.go::TestEvaluateRollbackCutoverUsesEffectiveAuthority`; `::TestEvaluateRollbackOnlyMatchingAuthenticatedBaselineCanAssertClean`; `internal/signerapp/backupadmin/activation_generational_test.go::TestRollbackRefusedAfterPostActivationMutation`; `docs/formal/rotation_transition.tla::R4_DivergenceNeverErased` | Transition start pins the pre-rewrap decision, completion never creates a clean baseline for a diverged cutover, and rollback refuses any live inventory that differs from the effective authenticated authority. |
