...
| S13 | implemented | Canonical Filename Binding; S13 | Scan derives payload selector and skips filename mismatches in `internal/keys/keys.go`; canonical writers use `paths.KeyFilePath(identityID, selector)` without write-time directory preflight | `internal/keys/keys_test.go::TestScanKeysDirectoryWithMasterKey/filename_address_mismatch_is_skipped`; `internal/keys/save_test.go::TestSavePayloadAllowsCanonicalWriteWithNonCanonicalKeyPresent`; `internal/keymgmt/operations_test.go::TestImportKeyRestoresCanonicalPathWhenExistingKeyIsNonCanonical`; `internal/backup/restore_test.go::TestRestoreKeyWritesCanonicalPathWhenExistingKeyIsNonCanonical` | Open Question 3 resolved as filename binding; address-collision invalidation remains a defensive fallback. |
