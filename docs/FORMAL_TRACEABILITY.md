...
| K8 | **not implemented** | Every durable class has an explicit term-authority classification | `internal/rotationinventory`; `internal/rotationinventory/start.go`; `internal/rotationinventory/resume.go`; `internal/rotationinventory/complete.go`; `internal/signerapp/backupadmin/rollback_generation.go`; `internal/genstore/records.go`; `internal/genstore/validate.go`; `internal/policy/integrity.go`; `internal/noderole/integrity.go` (partial lifecycle) | `internal/rotationinventory/scan_test.go::TestScanClassifiesEveryK8DurableClass`; `start_test.go::TestStartRotationCommitsSnapshotAnchorsAndRootInOrder`; `resume_test.go::TestResumeRotationRewrapsOnlyPinnedMutableConsumers`; `complete_test.go::TestCompleteRotationClosesVerifiedTransition`; `::TestCompleteRotationRejectsUnpinnedFinalPath`; `::TestCompleteRotationPublishesBaselineBeforeRootAndSnapshotCleanup`; `baseline_test.go::TestEvaluateRollbackOnlyMatchingAuthenticatedBaselineCanAssertClean`; `internal/signerapp/backupadmin/rollback_generation_test.go::TestRollbackMintReconstructsAnchoredSourceUnderCurrentTerm`; `internal/genstore/genstore_test.go::TestGenerationInventoryAndSealAuthenticateMemberTerms`; `::TestAnchoredHistoricalSealAndExactMemberOpen`; wrong-context, wrong-term, sealed-plaintext mutation, recursive-snapshot, oversize, durability-failure, symlink, post-baseline mutation, and unknown-artifact negative controls; generation-seal and sidecar tests | The taxonomy, scanner, snapshot/baseline, historical anchors, guarded start, pinned resume, final path/authority verification, baseline-before-close ordering, atomic close, snapshot cleanup, and rollback consumption are implemented. Operator/runtime wiring remains before K8 is complete. |
