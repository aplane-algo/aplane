...
| K8 | **not implemented** | Every durable class has an explicit term-authority classification | `internal/genstore/records.go`; `internal/policy/integrity.go`; `internal/noderole/integrity.go` (partial foundation) | `internal/genstore/genstore_test.go::TestSealAuthenticatesExactManifestBytes`; `::TestSealRejectsForgedSecurityFieldsAndWrongAuthority`; policy/node-role sidecar term tests | Generation seal v2 and explicit-term sidecars are implemented, but the complete cross-artifact scanner, snapshot inventory, final path-set check, and mutation-tested durable-class matrix remain the pre-append gate. |
