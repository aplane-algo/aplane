...
| K8 | **not implemented** | Every durable class has an explicit term-authority classification | `internal/rotationinventory`; `internal/rotationinventory/start.go`; `internal/rotationinventory/resume.go`; `internal/genstore/records.go`; `internal/genstore/validate.go`; `internal/policy/integrity.go`; `internal/noderole/integrity.go` (partial lifecycle) | `internal/rotationinventory/scan_test.go::TestScanClassifiesEveryK8DurableClass`; `start_test.go::TestStartRotationCommitsSnapshotAnchorsAndRootInOrder`; `::TestStartRotationPinsPriorBaselineAsEffectiveAuthority`; `resume_test.go::TestResumeRotationRewrapsOnlyPinnedMutableConsumers`; `::TestResumeRotationRejectsRetiringTermInputThatDiffersFromSnapshot`; `::TestResumeRotationRetriesVisibleTargetAfterDirSyncFailure`; `snapshot_test.go::TestSnapshotWriteReadPinsExactEncryptedFile`; `baseline_test.go::TestBaselineWriteReadAndAuthority`; `::TestReconcileBaselineForPreflight`; `internal/genstore/genstore_test.go::TestGenerationInventoryAndSealAuthenticateMemberTerms`; `::TestAnchoredHistoricalSealAndExactMemberOpen`; wrong-context, wrong-term, sealed-plaintext mutation, recursive-snapshot, oversize, durability-failure, symlink, and unknown-artifact negative controls; generation-seal and sidecar tests | The taxonomy, scanner, sealed snapshot/baseline, exact root references, authenticated member terms, complete historical-anchor collection, snapshot-before-root transition start, and pinned-input idempotent resume are implemented. Completion-baseline ordering, rollback consumption, and final exact-path/target-authority verification remain before K8 is complete. |
| R1 | deferred | Live store data is never stranded on an unauthorized term during rotation | `internal/crypto/keyring.go::authorizesCurrentStateTerm`; `internal/rotationinventory/resume.go::ResumeRotation` | `internal/rotationinventory/resume_test.go::TestResumeRotationRewrapsOnlyPinnedMutableConsumers`; `::TestResumeRotationRetriesVisibleTargetAfterDirSyncFailure`; `docs/formal/rotation_transition.tla::R1_LiveDataNeverStranded` | Pending authority and crash-resumable rewrap are implemented. The completed lifecycle and operator entry point remain before the implementation claim is closed. |
| R2 | deferred | Only cutover-pinned objects may reach the target term | `internal/rotationinventory/snapshot.go`; `internal/rotationinventory/resume.go::ResumeRotation` | `internal/rotationinventory/resume_test.go::TestResumeRotationRejectsRetiringTermInputThatDiffersFromSnapshot`; `::TestResumeRotationRejectsChangedPinnedPlaintext`; `::TestResumeRotationRejectsSymlinkedPinnedInput`; `docs/formal/rotation_transition.tla::R2_OnlyPinnedObjectsReachNewTerm` | Resume promotes only exact snapshot inputs and authenticates target outputs on retry. Final exact-path/output-authority comparison and close remain before the implementation claim is closed. |
...
| R4 | deferred | Rotation never erases pre-cutover divergence | `internal/rotationinventory/baseline.go::EvaluateRollbackCutover`; `internal/rotationinventory/start.go::StartRotation` | `internal/rotationinventory/baseline_test.go::TestEvaluateRollbackCutoverUsesEffectiveAuthority`; `start_test.go::TestStartRotationPinsPriorBaselineAsEffectiveAuthority`; `docs/formal/rotation_transition.tla::R4_DivergenceNeverErased` | Transition start now pins the manifest/prior-baseline decision in the root-referenced snapshot; completion and rollback consumption remain. Implementation gates remain PHASE3_ONBOARDING items 2 and 9. |
