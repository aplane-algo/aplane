...
| K8 | **not implemented** | Every durable class has an explicit term-authority classification | `internal/rotationinventory`; `internal/genstore/records.go`; `internal/policy/integrity.go`; `internal/noderole/integrity.go` (partial foundation) | `internal/rotationinventory/scan_test.go::TestScanClassifiesEveryK8DurableClass`; wrong-context, wrong-term, sealed-plaintext mutation, and unknown-artifact negative controls; generation-seal and sidecar tests | The canonical taxonomy, settled-store cross-artifact scanner, exact-buffer context opening, and durable-class matrix are implemented. Snapshot serialization/root pinning, term-bearing seal entries, anchored retired-term historical opening, and final exact-path/target-authority verification remain the pre-append gate. |
