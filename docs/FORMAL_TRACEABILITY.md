...
| R3 | deferred | A completed clean rotation leaves rollback available | `internal/rotationinventory/baseline.go`; `internal/rotationinventory/complete.go::CompleteRotation`; rollback wiring deferred | `internal/rotationinventory/complete_test.go::TestCompleteRotationWritesCleanCutoverBaselineBeforeClose`; `::TestCompleteRotationPublishesBaselineBeforeRootAndSnapshotCleanup`; `docs/formal/rotation_transition.tla::R3_CompletedRotationLeavesRollbackAvailable` | Completion durably writes and verifies the clean post-rewrap baseline before close. The rollback path must still consume that matching authority before the end-to-end claim is complete. |
| R4 | deferred | Rotation never erases pre-cutover divergence | `internal/rotationinventory/baseline.go::EvaluateRollbackCutover`; `internal/rotationinventory/start.go::StartRotation`; `internal/rotationinventory/complete.go::CompleteRotation` | `internal/rotationinventory/complete_test.go::TestCompleteRotationDoesNotEraseCutoverDivergence`; `internal/rotationinventory/baseline_test.go::TestEvaluateRollbackCutoverUsesEffectiveAuthority`; `docs/formal/rotation_transition.tla::R4_DivergenceNeverErased` | Transition start pins the pre-rewrap decision and completion refuses to create a baseline for a diverged cutover. Rollback consumption remains before the end-to-end claim is complete. |
