...
| S2 | implemented | LogicSig v1 Metadata; S2 | Sign-time: `internal/signerapp/signing/execution.go:212,359` (`SigningMetadataVersion == 0` -> `missingLogicSigSigningMetadata`); Restore-time: `internal/backup/restore.go` | Restore path: `internal/backup/restore_test.go::TestRestoreKeyRejectsLogicSigWithoutSigningMetadata`; sign path: `internal/signerapp/signing/execution_test.go::TestExecutorSignGenericLSigRejectsMissingSigningMetadata`; `*AssembleDSALogicSigRejectsMissingSigningMetadata` | |
...
| K6 | implemented | Resident terms do not imply current-state authority | `internal/crypto/keyring.go::OpenKeyring`; `::authorizesCurrentStateTerm`; `internal/crypto/policy_integrity.go::VerifyIntegrity` | `internal/crypto/keyring_test.go::TestOpenKeyringAcceptsSettledMultipleTermsButDeniesRetiredAuthority`; `internal/crypto/keyring_rotation_test.go::TestPendingAuthoritySetExcludesOlderResidentTerms` | Settled authority is exactly current; pending authority is exactly current plus `from_term`. Historical generation access is separately anchor-gated. |
...
| K8 | implemented | Every durable class has an explicit term-authority classification | `internal/rotationinventory`; `internal/storepass/rotate.go`; `internal/signerapp/storeadmin/service.go::ChangeStorePassphrase`; `internal/signerapp/daemon/admin_services.go::completePendingRotation`; `internal/signerapp/backupadmin/rollback_generation.go`; `internal/genstore/records.go`; `internal/genstore/validate.go`; `internal/policy/integrity.go`; `internal/noderole/integrity.go` | `internal/rotationinventory/scan_test.go::TestScanClassifiesEveryK8DurableClass`; `start_test.go::TestStartRotationCommitsSnapshotAnchorsAndRootInOrder`; `resume_test.go::TestResumeRotationRewrapsOnlyPinnedMutableConsumers`; `complete_test.go::TestCompleteRotationClosesVerifiedTransition`; `::TestCompleteRotationDiscardsPreRootSnapshotOrphan`; `::TestCompleteRotationIgnoresStaleSealOnCurrentGeneration`; `::TestCompleteRotationRejectsUnpinnedFinalPath`; `::TestCompleteRotationPublishesBaselineBeforeRootAndSnapshotCleanup`; `baseline_test.go::TestEvaluateRollbackOnlyMatchingAuthenticatedBaselineCanAssertClean`; `internal/storepass/rotate_test.go::TestRotateReencryptsKeysTemplatesAndMetadata`; `::TestRotateTreatsHelperFailureAsPostCommitWarning`; `::TestRotateAnchorsPriorGenerationsWithoutPruning`; `internal/signerapp/daemon/unlock_reconcile_test.go::TestUnlockCompletesPendingKeyRotationBeforePublishingIdentity`; `::TestUnlockEntersRecoveryWhenPendingRotationCannotComplete`; `internal/signerapp/backupadmin/rollback_generation_test.go::TestRollbackMintReconstructsAnchoredSourceUnderCurrentTerm`; wrong-context, wrong-term, sealed-plaintext mutation, recursive-snapshot, oversize, durability-failure, symlink, post-baseline mutation, and unknown-artifact negative controls; generation-seal and sidecar tests | Taxonomy, lifecycle, operator start, automatic pre-publication resume, fail-closed recovery, rollback consumption, and retained-generation reporting are wired end to end. Pre-root snapshot residue is removed only under a confirmed settled root, and non-authoritative seals beside `CURRENT` are excluded from rotation. |
| R1 | implemented | Live store data is never stranded on an unauthorized term during rotation | `internal/crypto/keyring.go::authorizesCurrentStateTerm`; `internal/rotationinventory/resume.go::ResumeRotation`; `internal/rotationinventory/complete.go::CompleteRotation`; `internal/crypto/keyring_store.go::CloseRotation` | `internal/rotationinventory/complete_test.go::TestCompleteRotationClosesVerifiedTransition`; `::TestCompleteRotationRecoversVisibleCloseBeforeSnapshotCleanup`; `internal/crypto/keyring_rotation_test.go::TestCloseRotationCommitsSettledAuthority`; `docs/formal/rotation_transition.tla::R1_LiveDataNeverStranded` | Pending authority keeps retiring inputs readable, resume promotes them, final verification rejects untransformed inputs, and close retires ordinary authority only after every pinned live consumer has target authority. Anchored historical generations keep their separate retained-term read path. |
| R2 | implemented | Only cutover-pinned objects may reach the target term | `internal/rotationinventory/snapshot.go`; `internal/rotationinventory/resume.go::ResumeRotation`; `internal/rotationinventory/complete.go::verifyCompletionInventory` | `internal/rotationinventory/resume_test.go::TestResumeRotationRejectsRetiringTermInputThatDiffersFromSnapshot`; `::TestResumeRotationRejectsChangedPinnedPlaintext`; `::TestResumeRotationRejectsSymlinkedPinnedInput`; `complete_test.go::TestCompleteRotationRejectsUnpinnedFinalPath`; `docs/formal/rotation_transition.tla::R2_OnlyPinnedObjectsReachNewTerm` | Resume promotes only exact snapshot inputs and authenticates target outputs; completion rejects any unpinned path and requires each pinned output's target-term kind/context before close. The clean completion baseline is the one explicit post-cutover output and must match the final generation inventory. |
| R3 | implemented | A completed clean rotation leaves rollback available | `internal/rotationinventory/baseline.go::EvaluateRollback`; `internal/rotationinventory/complete.go::CompleteRotation`; `internal/signerapp/backupadmin/activation_generational.go::rollbackRecoveredGenerational` | `internal/rotationinventory/complete_test.go::TestCompleteRotationWritesCleanCutoverBaselineBeforeClose`; `::TestCompleteRotationPublishesBaselineBeforeRootAndSnapshotCleanup`; `internal/rotationinventory/baseline_test.go::TestEvaluateRollbackOnlyMatchingAuthenticatedBaselineCanAssertClean`; `internal/signerapp/backupadmin/activation_generational_test.go::TestRollbackConsumesMatchingRotationBaselineAndRemovesItAfterMint`; `docs/formal/rotation_transition.tla::R3_CompletedRotationLeavesRollbackAvailable` | Completion durably writes and verifies the clean post-rewrap baseline before close, and rollback consumes only that matching current-term authority before reconstructing the target into a fresh generation. |
| R4 | implemented | Rotation never erases pre-cutover divergence | `internal/rotationinventory/scan.go::ScanForSnapshot`; `internal/rotationinventory/baseline.go::EvaluateRollbackCutover`; `::EvaluateRollback`; `internal/rotationinventory/start.go::StartRotation`; `internal/rotationinventory/complete.go::CompleteRotation` | `internal/rotationinventory/scan_test.go::TestScanRetainsCurrentDecisionInputsFromExactScannedBytes`; `internal/rotationinventory/complete_test.go::TestCompleteRotationDoesNotEraseCutoverDivergence`; `internal/rotationinventory/baseline_test.go::TestEvaluateRollbackCutoverUsesEffectiveAuthority`; `::TestEvaluateRollbackOnlyMatchingAuthenticatedBaselineCanAssertClean`; `internal/signerapp/backupadmin/activation_generational_test.go::TestRollbackRefusedAfterPostActivationMutation`; `docs/formal/rotation_transition.tla::R4_DivergenceNeverErased` | Transition start derives the pre-rewrap decision from the exact manifest and generation buffers already pinned by the scan, completion derives a clean baseline from the same scan-buffer authority, never creates one for a diverged cutover, and rollback refuses any live inventory that differs from the effective authenticated authority. |
| R5 | implemented | Resume never appends a second term to a pending rotation | `internal/crypto/keyring_store.go::StartRotation`; `internal/rotationinventory/start.go::StartRotation` | `internal/crypto/keyring_rotation_test.go::TestStartRotationCommitsPendingAuthorityAndR5Guard`; `internal/rotationinventory/start_test.go::TestStartRotationCommitsSnapshotAnchorsAndRootInOrder`; `docs/formal/rotation_transition.tla::R5_NoSecondAppend`; `rotation_transition_negative.cfg` | The descriptor guard runs before snapshot construction or durable writes and remains effective after reopening the pending root. The model's unguarded-resume mutation still supplies the expected third-term counterexample. |
