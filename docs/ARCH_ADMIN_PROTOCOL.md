...
- `key_details.parameters` for guarded account keys projects the embedded sentry verifier as `Sentry: <component selector>` and does not expose the raw `sentry_public_key` parameter
...
- `admin_settings`: `user_auto_approve`, `lock_on_disconnect`, `passphrase_timeout`, `passphrase_method`, `mode`, `ssh_enabled`, optional `ssh_port`, `ssh_fingerprint`, `ssh_clients`, `signer_port`, `teal_compile_network`, optional `endpoint_advertise_url`, `theme`
