...
| `key_overrides` | `identities/<identity>/policy.yaml`, `identities/<identity>/attestation.yaml` | unset | YAML-only sparse policy overrides. `policy.yaml` overrides are keyed by signing auth address; `attestation.yaml` overrides are keyed by attestor component selector. Unset fields inherit identity-wide policy, and nested overrides are rejected. |
