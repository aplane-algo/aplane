...
| `key_overrides` | `identities/<identity>/policy.yaml` | unset | YAML-only sparse policy overrides. Signer-domain overrides are keyed by signing auth address; sentry-domain overrides are keyed by sentry component selector. Unset fields inherit identity-wide policy, and nested overrides are rejected. |
