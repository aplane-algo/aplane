...
| Default-enabled attestor component key type | Raw component key generator/signing support is registered and cataloged as default-enabled. Examples are `aplane.sen-ed25519.v1` and `aplane.sen-falcon1024.v1`. | Yes on attestor nodes. | Component-signing only on attestor nodes; never normal spending `/sign`. | None. |
