...
| Sentry key | component-sign authority | `.key` category `component`, key type `aplane.sentry-*` | raw sentry signing key | `internal/keygen`, `internal/signing`, `internal/signerapp/signing` | `/sign` rejects; sentry-role `/sign/component` only; not a spending account. |
| Sentry Key ID | public selector | 52-character uppercase base32 SHA-512/256 of domain-separated key type and public key bytes | sentry key row `address` and compatibility `component_key` fields | `internal/sentry/keytypes` | Uniform for Ed25519/Falcon; txid-shaped but not a valid Algorand address; rejected as sender/auth/rekey/destination where address is expected. |
| Sentry public metadata sidecar | public metadata | `keys/<sentry_key_id>.public.json` | `apstore sentry export` source | `internal/keys`, `internal/sentry/sentryrefs` | Sentry Key ID/key type/public key consistency verified; no private material. |
...
| Plaintext template library | install source | repo or signer-data `library/templates/*.yaml` | KeyType Library candidates | `internal/templatelibrary` | Not active by presence alone; new signer identities install the bundled Falcon whitelist default, and invalid entries are reported but not enabled. |
...
| Sentry component audit projection | audit projection | component-signing outcome | `SIGN_APPROVED`/`SIGN_REJECTED` rows | `internal/signerapp/signing`, `internal/signerapp/audit` | Current MVP uses existing sign events; Sentry Key ID is `txn_auth`, decoded sender is `txn_sender`. |
