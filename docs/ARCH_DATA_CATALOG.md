...
| Backup import residue | ephemeral validation state | `backups/<identity>/.import-*.part`, `.import-claimed-*.part`, `.import-validation-*/` | unpublished upload, immutable validation claim, and same-filesystem deep-validation workspace | `internal/signerapp/backupadmin` | Owner-private, excluded from managed inventory, removed on normal completion or daemon startup. |
