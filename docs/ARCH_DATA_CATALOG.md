...
| Backup import residue | ephemeral validation state | `backups/<identity>/.import-*.part`, `.import-validation-*/` | unpublished upload and same-filesystem deep-validation workspace | `internal/signerapp/backupadmin` | Owner-private, excluded from managed inventory, removed on normal completion or daemon startup. |
