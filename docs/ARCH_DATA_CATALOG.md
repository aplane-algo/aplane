...
| Managed backup source settings | advisory backup metadata | optional `source_settings.json` inside managed archives | unverified source approval default and custom genesis-hash mappings | `internal/backup`, `internal/backup/sourcecontext` | Independent schema `aplane.backup.source-settings.v1`; no network credentials; missing/invalid metadata does not block valid key recovery. |
...
| Recovered batch root | inactive encrypted recovery and reconciliation state | `identities/<identity>/recovered/<restore-id>/` | no signing-runtime projection before activation; an `activation/` marker forces recovery mode and blocks signing | `internal/backup/recovered`, `internal/signerapp/backupadmin`, `internal/storepass` | Batch and entries are authenticated encryption under the destination master key; activation journal/rollback data are durable before active writes; passphrase rotation re-encrypts published state and refuses unresolved activation; `.recovering-*` directories are unpublished staging state. |
...
| Element | Kind | Authority | Projection | Owner | Checks |
|---|---|---|---|---|---|
| Managed credential envelope | authoritative secret storage | `keys/*.key` or `keys/*.sen` encrypted JSON | decrypted canonical payload | `internal/keys`, `internal/keystore` | Category selects the sole filename class; envelope and payload versions checked before use. |
...
| Backup/restore messages | wire contract | admin backup plus preview/recover/list/review/activate/rollback/purge DTOs | backup admin service calls | `internal/protocol`, `internal/signerapp/adminserver`, `internal/signerapp/backupadmin` | Admin protocol 3.1 adds typed source-settings review context; v3 retains conservative unknown compatibility entries and omits direct live restore; export passphrases are parsed as `SensitiveBytes`. |
...
| Backup source-settings sidecar | advisory packaged metadata | optional `source_settings.json` schema `aplane.backup.source-settings.v1` | missing/invalid/unverified recovered source context | `internal/backup`, `internal/backup/sourcecontext` | 256 KiB/1024-mapping bounds; canonical custom mappings only; never destination authority. |
...
| Activation review | request-scoped runtime model | recovered batch plus current policy/config/active fingerprints | security-first comparison, unverified source context, and review token | `internal/signerapp/backupadmin` | Token changes with destination policy, approval mode, source-settings status/digest, entries, or active conflicts. |
| Activation journal | durable reconciliation model | encrypted intent plus exact rollback snapshot | recovery-only runtime state | `internal/backup/recovered`, `internal/signerapp/backupadmin` | Published before active writes; resume and rollback are idempotent; purge rejects it. |
