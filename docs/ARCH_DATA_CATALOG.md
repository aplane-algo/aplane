...
| Sentry witness key | component-sign authority | `.key` category `witness`, key type `aplane.witness-*` | raw sentry-role witness key | `internal/keygen`, `internal/signing`, `internal/signerapp/signing` | `/sign` rejects; sentry-role `/sign/component` only; not a spending account. |
