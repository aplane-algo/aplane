...
| Identity mode | authoritative config field | identity `config.yaml` `mode` | key-class and service-dispatch gates | `internal/signerapp/identity`, `internal/signerapp/rest` | Values: `signing`, `attestation`, `dual`; tightening rejects conflicting active keys. |
...
| Component public metadata sidecar | public metadata | `keys/<a_selector>.public.json` | `apstore attestor export-public/list` source | `internal/keys`, `internal/attestor/attrefs` | Selector/key type/public key consistency verified; no private material. |
