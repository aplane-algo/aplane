...
- endpoint records carry connection profile fields together: `url` (`ssh://host[:port]`, loopback `http://...`, `https://...`, or `self` where supported), `signer_port`, `local_port`, `identity_file`, `known_hosts_path`, `token_file`, and endpoint-published `published_attestors`. Relative file paths resolve against `APCLIENT_DATA`.
...
- `apshell endpoints discover-attestors [--dry-run]` scans configured endpoints with authenticated `/keys`, extracts attestor component-key `public_key_hex` values, validates each `component_key`, and atomically rebuilds endpoint-local `published_attestors` inventory in `endpoints.yaml`. Reachable endpoints are refreshed; endpoints that cannot be queried preserve their existing `published_attestors` entries. Discovery still rejects duplicate public keys advertised by multiple endpoint aliases and leaves files unchanged on validation failures.
