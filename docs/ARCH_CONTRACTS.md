...
- endpoint records carry connection profile fields together: `url` (`ssh://host[:port]`, loopback `http://...`, `https://...`, or `self` where supported), `signer_port`, `local_port`, `identity_file`, `known_hosts_path`, and `token_file`. Relative file paths resolve against `APCLIENT_DATA`.
...
- `apshell endpoints discover-attestors [--dry-run]` scans every configured endpoint with authenticated `/keys`, extracts attestor component-key `public_key_hex` values, and atomically rebuilds alias-managed `attestor_endpoints` routes in `config.yaml`. Discovery preserves inline legacy routes, rejects duplicate public keys advertised by multiple endpoint aliases, rejects collisions with inline routes, and leaves files unchanged on any endpoint/query/validation failure.
