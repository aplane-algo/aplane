...
- `apshell endpoints attestors` lists the local endpoint-discovered attestor inventory by component ID without calling remote endpoints.
- `apshell endpoints list`, `endpoints show <alias>`, `endpoints default <alias>`, and `endpoints delete <alias>` operate on local client configuration. `show` is local-only and does not call `/keys`; `delete` refuses to remove the signer endpoint or an endpoint with published attestors still referenced by derived runtime routing.
