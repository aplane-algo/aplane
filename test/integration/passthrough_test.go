...
func TestPassthroughMixedGroup(t *testing.T) {
	...
	// - Txn A: Will be signed by apsigner (sign mode)
	...
	groupReq := signerapi.GroupSignRequest{
...
// 1. Sign a 2-txn group (account1 <-> account2, 0 ALGO) through apsigner
...
func TestPassthroughResign(t *testing.T) {
	...
	// Step 1: Sign the full group through apsigner
	...
	resignReq := signerapi.GroupSignRequest{
		Requests: []signerapi.SignRequest{
			{
	...
	var resignResp signerapi.GroupSignResponse
