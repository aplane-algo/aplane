...
// applugin-checksum generates checksums.sha256 for apshell external plugins.
//
// Usage:
//
//	applugin-checksum <plugin-directory> [additional-files...]
//
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: applugin-checksum <plugin-directory> [additional-files...]")
		...
		fmt.Fprintln(os.Stderr, "  applugin-checksum ./my-plugin")
		fmt.Fprintln(os.Stderr, "  applugin-checksum ./my-plugin lib/helper.js config.yaml")
		os.Exit(2)
	}
