...
func apstoreUsage() {
	...
	fmt.Fprintf(os.Stderr, "  apstore [-d path] restore apply <backup-id|name> [--address ADDRESS ...] [--replace-existing]\n")
	fmt.Fprintf(os.Stderr, "  apstore [-d path] restore rollback\n")
	fmt.Fprintf(os.Stderr, "  apstore [-d path] restore reconcile\n")
	...
	fmt.Fprintf(os.Stderr, "  apstore restore reconcile\n")
