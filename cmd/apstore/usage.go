...
func apstoreUsage() {
	...
	fmt.Fprintf(os.Stderr, "  apstore [-d path] endpoints export --alias <alias> --role signing|attestation|dual --url <url> [--out endpoint.json]\n")
	...
	fmt.Fprintf(os.Stderr, "  apstore endpoints export --alias attestor-local --role attestation --url ssh://127.0.0.1:2223 --out attestor-local.endpoint.json\n")
